(function (global) {
    ...
    Frontend.prototype = {
        ...
        checkingFileDownloadReady: function(){
        ...
        addProgressBarForPaypalButton:function(){
            var that = this;
        ...
        addAutoCompleteBankInput:function(){
        ...
        addEventDatetimePicker:function(){
        ...
        clickCopyClipboard:function(){
            $('.label-link').on('click',function(e){
                ...
                $.growl({ title: "Đã copy link affiliate!",message:'' });
                // $.growl.notice({message: "Copied the text: " + linkAffiliate});
            })
        },
        checkFreeDownload: function () {
...
//                        }
                        if (result.login) {
                            ...
                            $.growl.warning({message: "Bạn chưa đăng nhập!"});
                        ...
                        jQuery.ajax({
                            ...
                            success: function (res) {
                                ...
                                if (res.success) {
                                    ...
                                    if (res.edit) {
                                        $.growl.notice({message: (typeof (res.message) != "undefined") ? res.message : "Sửa bản ghi thành công"});
                                        location.href = "/admin/listing/" + MODEL
                                    } else {
                                        $.growl.notice({message: (typeof (res.message) != "undefined") ? res.message : "Thêm bản ghi thành công"});
                                        location.reload()
                                    }
        ...
        chooseCollectionSearch: function () {
        ...
        clickTurnOnModal: function () {
            ...
            $('#product-detail-modal').on('shown.bs.modal', function () {
                (document.getElementsByClassName('modal-content'))[0].scrollTo({top: 0, behavior: 'smooth'})
...
//                            }, "500")
                            ...
                            $.growl.notice({message: "Đã thêm sản phẩm vào giỏ hàng"});
                            setTimeout(() => {
                            ...
                            $(".total_item").removeClass("total_item_shake");
                            $('.loadingio-spinner-spinner-ry4x170fish').hide()
                            $.growl.warning({message: "Sản phẩm đã tồn tại trong giỏ hàng!"});
                        }
                    }
                });
            })
        ...
        clickClearProduct: function () {
            $('#clear_product').click(function () {
                ...
                if ($("input[name='product_check']:checked").length > 0) {
                    ...
                    $.growl.warning({message: "Bạn chưa chọn sản phẩm nào để xóa!"});
                }
            });
        },
